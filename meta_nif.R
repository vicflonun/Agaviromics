# CRATES ABUDANCE PLOTS FOR nif GENES #
...
#all
guild=list(name="Ncycling",genes=c("Nitrogen metabolism [PATH:ko00910]"))
function_id=data.frame()
for (i in guild$genes) {function_id=unique(rbind(function_id,path.data[path.data$pathway==i,c("ko_id","anygene_name")] ))}
ids=function_id$anygene_name ; names(ids)=function_id$ko_id

#by process
markers=list(NFix=c("nifH","nifD","nifK","anfG","vnfD","vnfK","vnfH","vnfG"))
function_id=data.frame()
...
orden2=c(markers$NFix)
orden2=orden2[orden2 %in% rownames(heat)]

#PLOT
png(filename = paste(result.dir,"nfix_HEAT_2.png",sep = "/"), 
    width = 100, height = 100, units = "mm", pointsize = 15, bg = "white", res=300, type="cairo")

pheatmap(t(heat[orden2[1:3],]), color = c("white",sequential_hcl(palette = "Grays",n=3000,rev = T)), cluster_rows = F, cluster_cols = F,
...
result=result[result$func_id %in% function_id[function_id$anygene_name %in% markers$NFix, "ko_id"],]
...
result_norm=as.data.frame(t(t(data)/colSums(data)*100))
...
imp=grep("[:.:]rz",unique(result.tidy$assembled.library), value = T)
...
#save plot
png(filename = paste(result.dir,"nif_barras_2.png",sep = "/"), 
